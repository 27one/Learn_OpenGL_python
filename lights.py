...
def main():
    ...
    vao_cube = gl.glGenVertexArrays(1)
    ...
    vao_light = gl.glGenVertexArrays(1)
    ...
    while not glfw.window_should_close(window):
        ...
        lamp_shader.use()
        lamp_shader.set_vec3("objectColor", Vector3([1.0, 0.5, 0.31]))
        lamp_modelLoc = gl.glGetUniformLocation(shader.ID, "model")
        gl.glUniformMatrix4fv(modelLoc, 1, gl.GL_FALSE, glm.value_ptr(model))
        lamp_shader.set_vec3("lightColor", Vector3([1.0, 1.0, 1.0]))
    ...
    gl.glDeleteVertexArrays(1, id(vao_cube))
